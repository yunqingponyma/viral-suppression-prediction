---
title: "XGBoost methods for R01 Project"
...
for (iter in 1:30) {
  ...
  # prepare the data
  dtrain <- xgb.DMatrix(data = train.tree %>%
                            dplyr::select(-VS) %>%
                            mutate_all(as.numeric) %>%
                            as.matrix(),
                        label = as.numeric(train.tree$VS) - 1)
  dtest <- xgb.DMatrix(data = test.tree %>%
                            dplyr::select(-VS) %>%
                            mutate_all(as.numeric) %>%
                            as.matrix(),
                        label = as.numeric(test.tree$VS) - 1)
  
  watchlist <- list(train=dtrain, test=dtest)
  
  # fit the model
  bst <- xgb.train(data=dtrain, max.depth=20, eta=1, nthread = 28,
                 nrounds=300, watchlist=watchlist, eval.metric = "error",
                 eval.metric = "logloss", objective = "binary:logistic", verbose = 0)
  
  XG.best.list[[iter]] <- bst
  
  y.prob <- predict(bst, dtest)
  y.hat <- as.numeric(y.prob > 0.5)
  y <- test.tree$VS
...
```{r}
save(res.matrix.list, d_te.list, XG.best.list, y.prob.list, y.true.list,
     file = 'XG_30_Lag1.rda')
```
...
for (iter in 1:30) {
  ...
  # fit the model
  bst <- xgb.train(data=dtrain, max.depth=20, eta=1, nthread = 14,
                 nrounds=300, watchlist=watchlist, eval.metric = "error",
                 eval.metric = "logloss", objective = "binary:logistic", verbose = 0)
