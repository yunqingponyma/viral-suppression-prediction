...
for (iter in 1:30) {
  ...
  train.RF <- d_lag_tr %>% mutate(VS = as.factor(VS))
  test.RF <- d_lag_te %>% mutate(VS = as.factor(VS))
  
  num_vars <- 5:15
  num_nodes <- 5:30
  
  tst.acc <-
    foreach(m = 1:length(num_vars),
            .combine = rbind,
            .packages = 'randomForest') %:%
    foreach(n = 1:length(num_nodes), .combine = c) %dopar% {
      RF <- randomForest(VS ~ ., data = train.RF[sample(nrow(train.RF), nrow(train.RF)*0.5),],
                         ntree = 1000, mtry = num_vars[m], maxnodes = num_nodes[n])
      yhat.oob <- RF$predicted
      mean(yhat.oob == RF$y)
    }
  
  tst.acc.list[[iter]] <- tst.acc
  
  print(paste0('iter =', iter, '. Max test accuracy: ', max(tst.acc), '.'))
  
  m_n <- which(tst.acc == max(tst.acc, na.rm = TRUE), arr.ind = T)
  M <- m_n[1]
  N <- m_n[2]
  
  RF.best <- randomForest(VS ~ ., data = train.RF[sample(nrow(train.RF), nrow(train.RF)*0.5),],
                          ntree = 1000, mtry = num_vars[M], maxnodes = num_nodes[N], importance = T)

  RF.best.list[[iter]] <- RF.best
  
  y.prob <- predict(RF.best, test.RF, type = 'prob')
  y.hat <- predict(RF.best, test.RF)
  y <- test.RF$VS
  ...
  importance_plot <- varImpPlot(RF.best)
  
  importance.list[[iter]] <- importance
  importance_plot.list[[iter]] <- importance_plot
  
}
```

```{r}
save(tst.acc.list, res.matrix.list, importance.list, importance_plot.list,
     d_te.list, RF.best.list, y.prob.list, y.true.list,
     file = 'RF_30_Lag1.rda')
```
